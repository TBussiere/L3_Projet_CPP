...
void list_move(int board[6][6], int nbVoiture) {
	for (int id = 1; id <= nbVoiture; id++) {
		...
		if (ori) {
			...
			std::cout << x << "/" << y << "/" << l << std::endl;
			while(board[x+l+a][y] == 0 && x+l+a < 6) {
				std::cout << "cout possible: " << id << "/" << a << " | " << x+l+a << std::endl;
				a++;
			}
		}
	}
...
int main() {
	...
	addVoiture(board, 2, false, 3, 1, 1);
